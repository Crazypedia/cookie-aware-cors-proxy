...
| `UPSTREAM_HEADER_PASSTHROUGH_EXTRA` | *(empty)* | Comma-separated list of extra upstream response headers (case-insensitive) to forward to the client, in addition to the built-in allowlist (`content-type`, `content-length`, `content-encoding`, `cache-control`, `last-modified`, `etag`, `expires`, `set-cookie`, `location`, `www-authenticate`, `retry-after`, `x-ratelimit-*`). Headers not on the allowlist (e.g. upstream `access-control-*` or other internal headers) are dropped rather than forwarded. For an allowlisted origin, every forwarded header except `set-cookie` is also sent via `Access-Control-Expose-Headers`, so browser JS (e.g. `fetch()`) can read it cross-origin — `set-cookie` is never exposable via that header in any browser and is instead handled through the proxy's cookie-translation behavior. |
...
| `CACP_DEBUG` | `FALSE` | Set to `TRUE` to log full per-request detail (including forwarded headers such as `Authorization`/`Key`/`x-apikey`) to the server's own logs. Operator-only — restart the service to change it. There is no caller-side way to enable this (a `/debug` path prefix and `CACP_DEBUG` request header used to do so; both were removed since either let any caller force another in-flight request's credentials into plaintext logs). |
| `CACP_LOG` | `FALSE` | Set to `TRUE` to log just the request URLs. Operator-only, same as above — the caller-side `/log` path prefix and `CACP_LOG` request header were removed for the same reason. |
